...
public class SubscriptionNotificationService {
    ...
    public void sendRenewalConfirmation(Paiement subscription) {
        emailServiceAbonnement.sendSubscriptionRenewalConfirmation(
            subscription.getUtilisateur().getEmail(),
            subscription.getUtilisateur().getNom() + " " + subscription.getUtilisateur().getPrenom(),
            subscription.getPackAbonnement().getNom(),
            subscription.getPackAbonnement().getDuree() + " jours"
        );
    }
    ...
    public void sendTestNotification(String email, String username, String packName, int daysUntilExpiration) {
        if (daysUntilExpiration > 0) {
            emailServiceAbonnement.sendSubscriptionExpirationReminder(email, username, packName, daysUntilExpiration);
        } else {
            emailServiceAbonnement.sendSubscriptionExpiredNotification(email, username, packName);
        }
    }
} 